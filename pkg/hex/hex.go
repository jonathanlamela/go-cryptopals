...
func FromHexString(s string) (Hex, error) { return Hex{S: s}, nil }

func FromBytes(b []byte) (Hex, error) {
	dst := make([]byte, stdhex.EncodedLen(len(b)))
	stdhex.Encode(dst, b)
	return Hex{S: string(dst)}, nil
}

func (h Hex) Len() int { return len(h.S) }

func (h Hex) ToBytes() ([]byte, error) {
	s := strings.ToLower(h.S)
...
func (h Hex) String() string { return h.S }
